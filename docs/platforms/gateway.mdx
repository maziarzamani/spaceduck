...
|--------|----------|------|-------------|
| `GET` | `/api/health` | No | Version, uptime, provider status |
| `GET` | `/api/config` | Yes | Read current configuration |
| `PATCH` | `/api/config` | Yes | Update configuration (JSON Patch) |
| `GET` | `/api/memories` | Yes | List memories (with filters) |
| `GET` | `/api/memories/search?q=...` | Yes | Hybrid memory search |
| `DELETE` | `/api/memories/:id` | Yes | Delete a memory |
| `GET` | `/api/tasks` | Yes | List scheduled tasks |
| `POST` | `/api/tasks` | Yes | Create a task |
| `GET` | `/api/tasks/budget` | Yes | Daily/monthly spend summary |
| `GET` | `/api/skills` | Yes | List installed skills |
| `WS` | `/ws` | Token | Chat streaming, real-time events |

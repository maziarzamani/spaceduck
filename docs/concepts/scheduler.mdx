...
| Memory writes | 10 | Writes silently dropped |
...
Tasks can reference a `skillId` to run with a specific [skill's](/concepts/skills) instructions and tool restrictions. When a task runs with a skill:

- The agent only sees tools permitted by the skill's `toolAllow`/`toolDeny` lists (intersected with any task-level restrictions)
- Memory writes are tagged with the skill's ID for provenance
- The skill's budget fields merge with the task's budget (task-level overrides win)

See [Skills](/concepts/skills) for the SKILL.md format and [Security](/concepts/security) for how these layers compose.
